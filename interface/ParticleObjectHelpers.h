...
template<typename T> bool ParticleObjectHelpers::objHasGreaterScalarSumPt(T const& earlier, T const& later){
  auto deepDaus_earlier = earlier.getDaughters(); deepDaus_earlier.clear();
  auto deepDaus_later = deepDaus_earlier;
  earlier.getDeepDaughters(deepDaus_earlier, true);
  later.getDeepDaughters(deepDaus_later, true);
  double scsumpt_earlier=0; for (auto const& dau:deepDaus_earlier) scsumpt_earlier += dau->pt();
  double scsumpt_later=0; for (auto const& dau:deepDaus_later) scsumpt_later += dau->pt();
...
template<typename T> bool ParticleObjectHelpers::objHasGreaterScalarSumPt_ImmediateDaughters(T const& earlier, T const& later){
  auto const& shallowDaus_earlier = earlier.getDaughters();
  auto const& shallowDaus_later = later.getDaughters();
  double scsumpt_earlier=0; for (auto const& dau:shallowDaus_earlier) scsumpt_earlier += dau->pt();
  double scsumpt_later=0; for (auto const& dau:shallowDaus_later) scsumpt_later += dau->pt();
  return (scsumpt_earlier > scsumpt_later);
}
  ...
  while (!remaining_keys.empty() && !remaining_vals.empty()){
    ...
    for (auto it_key = remaining_keys.begin(); it_key != remaining_keys.end(); it_key++){
      ...
      for (auto it_val = remaining_vals.begin(); it_val != remaining_vals.end(); it_val++){
        ...
        double deltaR;
        HelperFunctions::deltaR<double>(pVal.eta(), pVal.phi(), pKey.eta(), pKey.phi(), deltaR);
  ...
  while (!remaining_keys.empty() && !remaining_vals.empty()){
    ...
    if (chosenKey!=remaining_keys.end() && chosenVal!=remaining_vals.end()){
      ...
      if (match_thr>0.){
        if (matchByDeltaR){
          double deltaR;
          HelperFunctions::deltaR<double>(val->eta(), val->phi(), key->eta(), key->phi(), deltaR);
          if (deltaR>=match_thr){
