...
template<typename T> void SimpleEntry::setNamedVal(TString const& strname, T const& val){
  std::unordered_map<TString, T>& theMap = this->getNamedMap<T>();
  theMap[strname] = val;
}
template<typename T> void SimpleEntry::getNamedVal(TString const& strname, T& val) const{
...
template void SimpleEntry::setNamedVal<type>(TString const& strname, type const& val); \
template void SimpleEntry::getNamedVal<type>(TString const& strname, type& val) const; \
