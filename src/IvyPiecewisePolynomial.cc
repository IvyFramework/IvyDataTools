...
IvyPiecewisePolynomial::IvyPiecewisePolynomial(const char* name, const char* title, const int nfcn_, const int polyndof_) :
...
IvyPiecewisePolynomial::IvyPiecewisePolynomial(const char* name, const char* title, RooAbsReal& xvar_, RooArgList const& parList_, const int nfcn_, const int polyndof_) :
  RooAbsReal(name, title),
...
double IvyPiecewisePolynomial::eval(double x, std::vector<double> const& par)const{
  ...
  if (testdet!=0){
    ...
    std::cerr << "IvyPiecewisePolynomial::eval: Something went wrong, and the determinant is 0!" << std::endl;
    return d_epsilon;
  }
}

double IvyPiecewisePolynomial::evaluate() const{
...
double IvyPiecewisePolynomial::evaluate(double* x, double* p) const{ // For calling in a TF1 object
...
ClassImp(IvyPiecewisePolynomial)
