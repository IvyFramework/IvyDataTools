...
ClassImp(IvyNCSpline_3D_fast)

IvyNCSpline_3D_fast::IvyNCSpline_3D_fast() :
  IvyNCSplineCore(),
  ...
  bcBeginY(IvyNCSplineCore::bcNaturalSpline), bcEndY(IvyNCSplineCore::bcNaturalSpline),
  bcBeginZ(IvyNCSplineCore::bcNaturalSpline), bcEndZ(IvyNCSplineCore::bcNaturalSpline),
  ...
  FcnList(inFcnList)
{
  if (npointsX()>1 && npointsY()>1 && npointsZ()>1){
    ...
    if (det==0.){
      coutE(InputArguments) << "IvyNCSpline_3D_fast::interpolateFcn: Matrix xA could not be inverted. Something is wrong with the x coordinates of points!" << endl;
      assert(0);
    }

    vector<vector<IvyNCSplineCore::T>> yA; getKappas(kappaY, 1); getAArray(kappaY, yA, bcBeginY, bcEndY);
    npoints=kappaY.size();
    ...
    if (det==0.){
      coutE(InputArguments) << "IvyNCSpline_3D_fast::interpolateFcn: Matrix yA could not be inverted. Something is wrong with the y coordinates of points!" << endl;
      assert(0);
    }

    vector<vector<IvyNCSplineCore::T>> zA; getKappas(kappaZ, 2); getAArray(kappaZ, zA, bcBeginZ, bcEndZ);
    npoints=kappaZ.size();
    ...
    if (det==0.){
      coutE(InputArguments) << "IvyNCSpline_3D_fast::interpolateFcn: Matrix zA could not be inverted. Something is wrong with the z coordinates of points!" << endl;
      assert(0);
    }

    //cout << "IvyNCSpline_3D_fast::IvyNCSpline_3D_fast: Initial kappa arrays are set up" << endl;
    ...
    std::vector<
      ...
      std::vector<std::vector<IvyNCSplineCore::T>>
      >>
    ...
    for (unsigned int k=0; k<npointsZ(); k++){
      ...
      vector<vector<vector<IvyNCSplineCore::T>>> coefsAlongY; // [xbin][Ax(y),Bx(y),Cx(y),Dx(y)][ybin] in each z
      for (unsigned int j=0; j<npointsY(); j++){
        vector<vector<IvyNCSplineCore::T>> xcoefsAtYjZk = getCoefficientsPerYPerZ(kappaX, xAinv, j, k, bcBeginX, bcEndX, -1); // [ix][Ax,Bx,Cx,Dx] at each y_j z_k
        ...
        if (j==0){
          ...
          for (int ix=0; ix<nxbins; ix++){
            vector<vector<IvyNCSplineCore::T>> dum_xycoefarray;
            for (int icx=0; icx<nxpoldim; icx++){
              vector<IvyNCSplineCore::T> dum_ycoefarray;
        ...
        if (nxbins!=(int)xcoefsAtYjZk.size() || nxpoldim!=(int)xcoefsAtYjZk.at(0).size()){
          coutE(InputArguments) << "IvyNCSpline_3D_fast::interpolateFcn: nxbins!=(int)xcoefsAtYjZk.size() || nxpoldim!=(int)xcoefsAtYjZk.at(0).size()!" << endl;
          assert(0);
        }
        for (int ix=0; ix<nxbins; ix++){
      ...
      for (int ix=0; ix<nxbins; ix++){
        ...
        vector<vector<vector<IvyNCSplineCore::T>>> xCoefs;
        for (int icx=0; icx<nxpoldim; icx++){
          vector<vector<IvyNCSplineCore::T>> yCoefs = getCoefficientsAlongDirection(kappaY, yAinv, coefsAlongY.at(ix).at(icx), bcBeginY, bcEndY, -1); // [iy][A,B,C,D]
      ...
      if (k==0){
        ...
        for (int ix=0; ix<nxbins; ix++){
          vector<vector<vector<vector<IvyNCSplineCore::T>>>> xCoefs;
          for (int icx=0; icx<nxpoldim; icx++){
            vector<vector<vector<IvyNCSplineCore::T>>> xCoefsAlongY;
            for (int iy=0; iy<nybins; iy++){
              vector<vector<IvyNCSplineCore::T>> yCoefs;
              for (int icy=0; icy<nypoldim; icy++){
                vector<IvyNCSplineCore::T> yCoefAtZj;
    ...
    for (int ix=0; ix<nxbins; ix++){
      vector<vector<vector<vector<vector<IvyNCSplineCore::T>>>>> xCoefs;
      for (int icx=0; icx<nxpoldim; icx++){
        vector<vector<vector<vector<IvyNCSplineCore::T>>>> xCoefsAlongY;
        for (int iy=0; iy<nybins; iy++){
          vector<vector<vector<IvyNCSplineCore::T>>> yCoefs;
          for (int icy=0; icy<nypoldim; icy++){
            vector<vector<IvyNCSplineCore::T>> yCoefsAlongZ = getCoefficientsAlongDirection(kappaZ, zAinv, coefsAlongZ.at(ix).at(icx).at(iy).at(icy), bcBeginZ, bcEndZ, -1); // [iz][A,B,C,D]
...
IvyNCSpline_3D_fast::IvyNCSpline_3D_fast(
  const IvyNCSpline_3D_fast& other,
  const char* name
  ) :
...
IvyNCSplineCore::T IvyNCSpline_3D_fast::interpolateFcn(Int_t code, const char* rangeName) const{
  DefaultAccumulator<IvyNCSplineCore::T> res;

  if (verbosity==IvyNCSplineCore::kVerbose) cout << "IvyNCSpline_3D_fast(" << GetName() << ")::interpolateFcn begin with code: " << code << endl;
  ...
  vector<IvyNCSplineCore::T> tvar;
  vector<IvyNCSplineCore::T> tvarmin;
  vector<IvyNCSplineCore::T> tvarmax;
  vector<const vector<IvyNCSplineCore::T>*> varkappa; varkappa.push_back(&kappaX); varkappa.push_back(&kappaY); varkappa.push_back(&kappaZ);
  ...
  for (Int_t idim=0; idim<ndims; idim++){
    ...
    Int_t binmin=-1;
    Int_t binmax=-1;
    IvyNCSplineCore::T tval=0;
    IvyNCSplineCore::T tmin=0;
    IvyNCSplineCore::T tmax=0;
    if (code==0 || code%varprime.at(idim)!=0){
      ...
      IvyNCSplineCore::T coordmin = varcoord.at(idim)->min(rangeName); cropValueForRange(coordmin, idim);
      IvyNCSplineCore::T coordmax = varcoord.at(idim)->max(rangeName); cropValueForRange(coordmax, idim);
      const std::vector<IvyNCSplineCore::T>& kappadim = *(varkappa.at(idim));
  ...
  for (int ix=0; ix<(int)coefficients.size(); ix++){
    ...
    // Get the x coefficients interpolated across y
    vector<IvyNCSplineCore::T> xCoefs;
    for (int icx=0; icx<(int)coefficients.at(ix).size(); icx++){
      DefaultAccumulator<IvyNCSplineCore::T> theXCoef;
      for (int iy=0; iy<(int)coefficients.at(ix).at(icx).size(); iy++){
        ...
        IvyNCSplineCore::T tylow=0, tyhigh=1;
        ...
        vector<IvyNCSplineCore::T> yCoefs;
        for (int icy=0; icy<(int)coefficients.at(ix).at(icx).at(iy).size(); icy++){
          DefaultAccumulator<IvyNCSplineCore::T> theYCoef;
          for (int iz=0; iz<(int)coefficients.at(ix).at(icx).at(iy).at(icy).size(); iz++){
            ...
            IvyNCSplineCore::T tzlow=0, tzhigh=1;
...
void IvyNCSpline_3D_fast::getKappas(vector<IvyNCSplineCore::T>& kappas, const Int_t whichDirection){
  kappas.clear();
...
Int_t IvyNCSpline_3D_fast::getWhichBin(const IvyNCSplineCore::T& val, const Int_t whichDirection) const{
  Int_t bin=-1;
...
IvyNCSplineCore::T IvyNCSpline_3D_fast::getTVar(const vector<IvyNCSplineCore::T>& kappas, const IvyNCSplineCore::T& val, const Int_t& bin, const Int_t whichDirection) const{
...
vector<vector<IvyNCSplineCore::T>> IvyNCSpline_3D_fast::getCoefficientsPerYPerZ(
  const std::vector<IvyNCSplineCore::T>& kappaX, const TMatrix_t& xAinv,
  const Int_t& ybin, const Int_t& zbin,
  IvyNCSplineCore::BoundaryCondition const& bcBegin, IvyNCSplineCore::BoundaryCondition const& bcEnd,
  const Int_t xbin
  ) const{
  vector<IvyNCSplineCore::T> fcnList;
  for (unsigned int bin=0; bin<npointsX(); bin++){ fcnList.push_back(FcnList.at(zbin).at(ybin).at(bin)); }
  vector<vector<IvyNCSplineCore::T>> coefs = getCoefficientsAlongDirection(kappaX, xAinv, fcnList, bcBegin, bcEnd, xbin);
  return coefs;
}

Double_t IvyNCSpline_3D_fast::evaluate() const{
  Double_t value = interpolateFcn(0);
  if (useFloor && value<floorEval){
    if (verbosity>=IvyNCSplineCore::kError) coutE(Eval) << "IvyNCSpline_3D_fast ERROR::IvyNCSpline_3D_fast(" << GetName() << ") evaluation returned " << value << " at (x, y, z) = (" << theXVar << ", " << theYVar << ", " << theZVar << ")" << endl;
    value = floorEval;
  }
  if (verbosity==IvyNCSplineCore::kVerbose){ cout << "IvyNCSpline_3D_fast(" << GetName() << ")::evaluate = " << value << " at (x, y, z) = (" << theXVar << ", " << theYVar << ", " << theZVar << ")" << endl; }
  return value;
}
Int_t IvyNCSpline_3D_fast::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* /*rangeName*/) const{
...
Double_t IvyNCSpline_3D_fast::analyticalIntegral(Int_t code, const char* rangeName) const{
  Double_t value = interpolateFcn(code, rangeName);
  if (useFloor && value<floorInt){
    if (verbosity>=IvyNCSplineCore::kError) coutE(Integration) << "IvyNCSpline_3D_fast ERROR::IvyNCSpline_3D_fast(" << GetName() << ") integration returned " << value << " for code = " << code << endl;
    value = floorInt;
  }
  if (verbosity==IvyNCSplineCore::kVerbose){ cout << "IvyNCSpline_3D_fast(" << GetName() << ")::analyticalIntegral = " << value << " for code = " << code << endl; }
  return value;
}

Bool_t IvyNCSpline_3D_fast::testRangeValidity(const T& val, const Int_t whichDirection) const{
  const T* range[2];
  if (whichDirection==0){
    range[0] = &rangeXmin;
    range[1] = &rangeXmax;
...
void IvyNCSpline_3D_fast::setRangeValidity(const T valmin, const T valmax, const Int_t whichDirection){
  T* range[2];
  if (whichDirection==0){
    range[0] = &rangeXmin;
    range[1] = &rangeXmax;
...
void IvyNCSpline_3D_fast::cropValueForRange(T& val, const Int_t whichDirection) const{
