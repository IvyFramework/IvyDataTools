...
ClassImp(IvyNCSpline_2D_fast)

IvyNCSpline_2D_fast::IvyNCSpline_2D_fast() :
  IvyNCSplineCore(),
  ...
  FcnList(inFcnList)
{
  if (npointsX()>1 && npointsY()>1){
    ...
    if (det==0.){
      coutE(InputArguments) << "IvyNCSpline_2D_fast::interpolateFcn: Matrix xA could not be inverted. Something is wrong with the x coordinates of points!" << endl;
      assert(0);
    }
    ...
    if (det==0.){
      coutE(InputArguments) << "IvyNCSpline_2D_fast::interpolateFcn: Matrix yA could not be inverted. Something is wrong with the y coordinates of points!" << endl;
      assert(0);
    }

    // Get the grid of coefficients
    vector<vector<vector<IvyNCSplineCore::T>>> coefsAlongY; // [Ax(y),Bx(y),Cx(y),Dx(y)][xbin][ybin]
    int npoldim=0;
    int nxbins=0;
    for (unsigned int j=0; j<npointsY(); j++){
      vector<vector<IvyNCSplineCore::T>> xcoefsAtYj = getCoefficientsPerY(kappaX, xAinv, j, bcBeginX, bcEndX, -1); // [ix][Ax,Bx,Cx,Dx] at each y_j
      if (j==0){
      ...
      if (nxbins!=(int)xcoefsAtYj.size() || npoldim!=(int)xcoefsAtYj.at(0).size()){
        coutE(InputArguments) << "IvyNCSpline_2D_fast::interpolateFcn: nxbins!=(int)xcoefsAtYj.size() || npoldim!=(int)xcoefsAtYj.at(0).size()!" << endl;
        assert(0);
      }
      for (int ix=0; ix<nxbins; ix++){
    ...
    for (int ix=0; ix<nxbins; ix++){
      ...
      for (int ic=0; ic<npoldim; ic++){
        vector<vector<IvyNCSplineCore::T>> yCoefs = getCoefficientsAlongDirection(kappaY, yAinv, coefsAlongY.at(ic).at(ix), bcBeginY, bcEndY, -1); // [iy][A,B,C,D]
...
IvyNCSpline_2D_fast::IvyNCSpline_2D_fast(
  const IvyNCSpline_2D_fast& other,
  const char* name
  ) :
...
IvyNCSplineCore::T IvyNCSpline_2D_fast::interpolateFcn(Int_t code, const char* rangeName) const{
  DefaultAccumulator<IvyNCSplineCore::T> res;

  if (verbosity==IvyNCSplineCore::kVerbose){ cout << "IvyNCSpline_2D_fast(" << GetName() << ")::interpolateFcn begin with code: " << code << endl; }

  // Get bins
  Int_t xbin=-1, xbinmin=-1, xbinmax=-1, ybin=-1, ybinmin=-1, ybinmax=-1;
  IvyNCSplineCore::T tx=0, txmin=0, txmax=0, ty=0, tymin=0, tymax=0;
  if (code==0 || code%2!=0){ // Case to just compute the value at x
    ...
    IvyNCSplineCore::T coordmin = theXVar.min(rangeName); cropValueForRange(coordmin, 0);
    IvyNCSplineCore::T coordmax = theXVar.max(rangeName); cropValueForRange(coordmax, 0);
  ...
  if (code==0 || code%3!=0){ // Case to just compute the value at y
    ...
    IvyNCSplineCore::T coordmin = theYVar.min(rangeName); cropValueForRange(coordmin, 1);
    IvyNCSplineCore::T coordmax = theYVar.max(rangeName); cropValueForRange(coordmax, 1);
  ...
  for (int ix=0; ix<(int)coefficients.size(); ix++){
    ...
    for (int ic=0; ic<(int)coefficients.at(ix).size(); ic++){
      const vector<vector<IvyNCSplineCore::T>>& yCoefs = coefficients.at(ix).at(ic);

      if (verbosity==IvyNCSplineCore::kVerbose) cout << "\tCoefficient " << ic << ":\n";

      DefaultAccumulator<IvyNCSplineCore::T> theCoef;
...
void IvyNCSpline_2D_fast::getKappas(vector<IvyNCSplineCore::T>& kappas, const Int_t whichDirection){
  kappas.clear();
...
Int_t IvyNCSpline_2D_fast::getWhichBin(const IvyNCSplineCore::T& val, const Int_t whichDirection) const{
  Int_t bin=-1;
...
IvyNCSplineCore::T IvyNCSpline_2D_fast::getTVar(const vector<IvyNCSplineCore::T>& kappas, const IvyNCSplineCore::T& val, const Int_t& bin, const Int_t whichDirection) const{
...
vector<vector<IvyNCSplineCore::T>> IvyNCSpline_2D_fast::getCoefficientsPerY(const std::vector<IvyNCSplineCore::T>& kappaX, const TMatrix_t& xAinv, const Int_t& ybin, IvyNCSplineCore::BoundaryCondition const& bcBegin, IvyNCSplineCore::BoundaryCondition const& bcEnd, const Int_t xbin) const{
...
Double_t IvyNCSpline_2D_fast::evaluate() const{
  Double_t value = interpolateFcn(0);
  if (useFloor && value<floorEval){
    if (verbosity>=IvyNCSplineCore::kError) coutE(Eval) << "IvyNCSpline_2D_fast ERROR::IvyNCSpline_2D_fast(" << GetName() << ") evaluation returned " << value << " at (x, y) = (" << theXVar << ", " << theYVar << ")" << endl;
    value = floorEval;
  }
  if (verbosity==IvyNCSplineCore::kVerbose){ cout << "IvyNCSpline_2D_fast(" << GetName() << ")::evaluate = " << value << " at (x, y) = (" << theXVar << ", " << theYVar << ")" << endl; }
  return value;
}
Int_t IvyNCSpline_2D_fast::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* /*rangeName*/) const{
...
Double_t IvyNCSpline_2D_fast::analyticalIntegral(Int_t code, const char* rangeName) const{
  Double_t value = interpolateFcn(code, rangeName);
  if (useFloor && value<floorInt){
    if (verbosity>=IvyNCSplineCore::kError) coutE(Integration) << "IvyNCSpline_2D_fast ERROR::IvyNCSpline_2D_fast(" << GetName() << ") integration returned " << value << " for code = " << code << endl;
    value = floorInt;
  }
  if (verbosity==IvyNCSplineCore::kVerbose){ cout << "IvyNCSpline_2D_fast(" << GetName() << ")::analyticalIntegral = " << value << " for code = " << code << endl; }
  return value;
}

Bool_t IvyNCSpline_2D_fast::testRangeValidity(const T& val, const Int_t whichDirection) const{
  const T* range[2];
  if (whichDirection==0){
    range[0] = &rangeXmin;
    range[1] = &rangeXmax;
...
void IvyNCSpline_2D_fast::setRangeValidity(const T valmin, const T valmax, const Int_t whichDirection){
  T* range[2];
  if (whichDirection==0){
    range[0] = &rangeXmin;
    range[1] = &rangeXmax;
...
void IvyNCSpline_2D_fast::cropValueForRange(T& val, const Int_t whichDirection) const{
